...
class Hifuse(nn.Module):
    def __init__(self, num_classes):
        ...
        self.classfier = HiFuse_Small(num_classes)
        self.linear = nn.Linear(768, num_classes)

    def forward(self, x):
        return self.linear(self.classfier(x))




class HifuseWithOur(nn.Module):
    """
    这个版本分别使用 image 和 text 的 embedding 作为 kqv 然后再将输出进行融合。
    """
    def __init__(self, args, num_labels=2, loss_class="textimage_loss"):

        super(HifuseWithOur, self).__init__()
        self.text_encoder = BertEncoder()
        
        self.image_encoder = HiFuse_Small(7)
        # self.conv = nn.Conv2d(in_channels=1024, out_channels=768, kernel_size=1)
        self.textbased_cross_attention = MultiHeadCrossAttention_v2(dim=768,num_heads=4)
        self.imagbased_cross_attention = MultiHeadCrossAttention_v2(dim=768,num_heads=4)
