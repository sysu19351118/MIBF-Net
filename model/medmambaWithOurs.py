...
import sys
sys.path.append('/home/zzixuantang/playground/mamba/MedMamba-main')
from MedMamba2 import VSSM

class MedMamba(nn.Module):
    def __init__(self, num_classes):
        super(MedMamba, self).__init__()
        self.encoder = VSSM(depths=[2, 2, 4, 2],dims=[96,192,384,768],num_classes=num_classes)
...
class MedMambaWithOur(nn.Module):
    """
    这个版本分别使用 image 和 text 的 embedding 作为 kqv 然后再将输出进行融合。
    """
    def __init__(self, num_labels=2, loss_class="textimage_loss"):

        super(MedMambaWithOur, self).__init__()
        self.text_encoder = BertEncoder()
        
        self.image_encoder = VSSM(depths=[2, 2, 4, 2],dims=[96,192,384,768],num_classes=num_labels)
